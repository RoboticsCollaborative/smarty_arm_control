...
void smartyArmControl (Rdda *rdda) {
    ...
    double upper_arm_init_offset = 0.9012; // motor 1
    double forearm_init_offset = 0.3027; // motor 2
    // double num = 3;
    ...
    for (int i = 0; i < ARM_NUM; i ++) {
        ...
        model[i].R[1][0] = model[i].c5 * (model[i].c4 * model[i].s0 + model[i].c0 * model[i].c3 * model[i].s4) - model[i].c0 * model[i].c3 * model[i].s5;
